...
class SequencePredictor(Model):
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.l2_loss(self.labels_placeholder-preds)
    ...
    def add_training_op(self, loss):
        ...
        gradients_var = optimizer.compute_gradients(loss)
        gradients = [tmp[0] for tmp in gradients_var]
        vars = [tmp[1] for tmp in gradients_var]

        if self.config.clip_gradients is True:
            gradients, _ = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
        self.grad_norm = tf.global_norm(gradients)
        gradients_new = zip(gradients,vars)
        train_op = optimizer.apply_gradients(gradients_new)
