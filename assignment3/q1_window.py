    ...
    for sentence, labels in data:
        for i in range(len(sentence)):
            ...
            for j in range(max(0,i-window_size),i):
                tmp+=sentence[j]
            for j in range(i,min(i+window_size+1,len(sentence))):
                tmp+=sentence[j]
            if i + window_size + 1 > len(sentence):
                for j in range(i+window_size+1-len(sentence)):
                    tmp+=end
            windowed_data.append((tmp,labels[i]))
