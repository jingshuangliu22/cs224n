...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        n_features, embedding_size = self.config.n_features, self.config.embed_size
        with tf.name_scope("embedding_layer"):
            pretrained_embeddings = tf.Variable(self.pretrained_embeddings, 'embedding')
    ...
    def add_prediction_op(self):
        ...
        with tf.name_scope("1_hidden_layer_NN"):
            W = tf.get_variable("W",[n_features*embed_size, hidden_size],initializer=xavier_weight_init())
            b1 = tf.Variable(tf.zeros([hidden_size,]), name="biases1")
            h = tf.nn.relu(tf.matmul(x, W) + b1)
            h_drop = tf.nn.dropout(h, self.dropout_placeholder)
            U = tf.get_variable("U", [hidden_size, n_classes], initializer=xavier_weight_init())
